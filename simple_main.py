...
# from fluentflet.window import FluentWindow, Title
...
class DisplayItem(ft.Container):
    def __init__(self, title: str = "", content: ft.Control = None, **kwargs):
        self.title = ft.Text(value=title, size=16, weight=ft.FontWeight.W_500)

        self.content_container = (
            ft.Container(content=content, expand=True) if content else None
        )

        super().__init__(
            content=ft.Column(
                [self.title, self.content_container] if content else [self.title],
                spacing=10,
                horizontal_alignment=ft.CrossAxisAlignment.START,
                expand=True,
            ),
...
def main(page: ft.Page):
    ...
    def increment_value(e):
        current = (
            float(text_value.value)
            if text_value.value.replace(".", "", 1).isdigit()
            else 0
        )
                    ...
                    [
                        ft.Row(
                            [
                                ...
                                ft.Text(
                                    "Fluent Flet", size=30, weight=ft.FontWeight.W_900
                                ),
                            ]
                        ),
                        ft.Row(
                                                ...
                                                [
                                                    ...
                                                    Slider(
                                                        ...
                                                        on_change=lambda e: print(
                                                            f"Value changed to: {round(e.current_value)}",
                                                            end="\r",
                                                        ),
                                                    ),
                                                ]
                                                ...
                                                [
                                                    Calendar(
                                                        on_select=lambda date: print(
                                                            f"Date selected: {date.strftime('%Y-%m-%d') if date else 'None'}"
                                                        )
                                                    )
                                                ]
                                            ),
                                        ...
                                        DisplayItem(
                                            ...
                                            content=ft.ListView(
                                                ...
                                                controls=[
                                                    ...
                                                    ListItem(
                                                        content=ft.Text("Henry Ross")
                                                    ),
                                                    ListItem(
                                                        content=ft.Text("Nicole Wagner")
                                                    ),
                                                ],
                                            ),
                                        ),
